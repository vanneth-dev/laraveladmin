...
if (! function_exists('hashids_encode')) {
    function hashids_encode($key, $field_id, $hash_length)
    {
        $hashids = new Hashids($key, $hash_length);

        return is_string($field_id) ? $hashids->encodeHex($field_id) : $hashids->encode($field_id);
    }
}

if (! function_exists('hashids_decode')) {
    function hashids_decode($key, $field_id, $hash_length)
    {
        $hashids = new Hashids($key, $hash_length);
        $decoded = $hashids->decode($field_id);
        $decodedHex = $hashids->decodeHex($field_id);

        return strpos($decodedHex, 'fffffabc') !== false ? $decodedHex : $decoded;
    }
}
