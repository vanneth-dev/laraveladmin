...
class GlobalHelper
{
    ...
    $moduleObj = DB::table('modules')->whereIn('id', $availableModules)->where('active', 'Y')->orderBy('order', 'desc')->whereNull('deleted_at')->get();
                ...
                if ($modulePages) {
                    foreach ($modulePages as $page) {
                        if (isset($actions[$page->id])) {
                            ...
                            $html .=
                            '<span class="item" style="line-height: 30px">
                            <i class="nav-icon ' . $page->icon . ' " style="font-size: 0.9em;"></i>
                            <i class="nav-icon "></i>
                            <span class="">' . $page->{'name' . $locale} . '</span>
                            </span>';
                            $html .= '</li>';
                        }
                    }
